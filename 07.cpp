...
bool expect(char c, std::string& input, int pos)
{
	...
	else
	{
		std::cout << "Test auf " << c << " nicht erfolgreich. "
				  << "Stattdessen " << input.at(pos) << " gesehen.\n";
		return false;
	}
}
...
void match(char c, std::string& input, int& pos)
{
	...
	if (input.at(pos) != c)
	{
		std::cout << "Fehler: an Position " << pos << " erwarte " << c << " und sehe "
				  << input.at(pos) << std::endl;
		std::cout << "Verlasse match(): " << std::endl;
		return;
	}
	pos++;
	std::cout << "Zeichen " << c << " konsumiert.\n";
	std::cout << "Verlasse match() fuer das Zeichen " << c << std::endl;
}
